...
class BottomNavigationViewWithIndicator : BottomNavigationView,
    ...
    //    private val bottomOffset = resources.getDimension(R.dimen.bottom_margin) // original
    private val bottomOffset = 0F
    ...
    public fun onItemSelected(itemId: Int, animate: Boolean = true) {
        if (!isLaidOut) return
...
//                val indicatorWidth = defaultSize * scale
                val indicatorWidth = defaultSize
//                val left = distanceTravelled - indicatorWidth / 2f
                val left = distanceTravelled - indicatorWidth
//                val top = height - bottomOffset - defaultSize // original
                val top = height - bottomOffset - defaultSize + 6 // original
//                val right = distanceTravelled + indicatorWidth / 2f
                val right = distanceTravelled + indicatorWidth
//                val bottom = height - bottomOffset // original
//                val bottom = height - bottomOffset - bottomOffset + 2
                val bottom = height + 10
//                val bottom = height

//                indicator.set(left, top, right, bottom)
                indicator.set(left, top, right, bottom.toFloat())
                invalidate()
            }
